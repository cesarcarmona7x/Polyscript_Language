...
namespace D2DUI{
	...
	public:
		...
		std::vector<FontStyleRange>rangesFontStyle;
		std::vector<FontColorRange>rangesColor;
	protected:
	...
	private:
		ComPtr<ID2D1BitmapBrush> brush_thumb,brush_track,brush_track_pressed;
	...
	public:
		ScrollLayout(Orientation o=VERTICAL);
		~ScrollLayout(){}
	...
	class LogElement:public TextLabel{
	public:
		LogElement(std::wstring text=L""):TextLabel(text),radiusX(0.f),radiusY(0.f){}
		~LogElement(){}
		virtual void setHovered(bool hovered){
			WindowBase::setHovered(hovered);
		}
		virtual bool isHovered(){
			return WindowBase::isHovered();
		}
		virtual void setPressed(bool pressed){
			WindowBase::setPressed(pressed);
		}
		virtual bool isPressed(){
			return WindowBase::isPressed();
		}
		virtual void setRadius(float radiusX,float radiusY=-1.f);
		virtual float getXRadius();
		virtual float getYRadius();
		virtual void recreateResources(std::shared_ptr<D2DHandle>& d2d);
		virtual void draw(std::shared_ptr<D2DHandle>& d2d);
